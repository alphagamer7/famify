...
CREATE POLICY "Users can view all profiles"
  ...
  -- Sensitive data should be in separate tables with proper RLS

-- Keep the update policy as-is
-- CREATE POLICY "Users can update their own profile"
--   ON profiles FOR UPDATE
--   USING (id = auth.uid());
-- (This policy is already correct, no need to drop/recreate)
...
-- FROM pg_policies
-- WHERE tablename IN ('profiles', 'family_members')
